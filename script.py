...
# Function to calculate scores with the new conditions
def calculate_scores(data):
    results = []

    for user_id, group in data.groupby("user_id"):
        # Ensure correct datetime conversion
        group["start_time"] = pd.to_datetime(group["start_time"], errors='coerce')
        group["end_time"] = pd.to_datetime(group["end_time"], errors='coerce')

        # Filter for valid lessons (90% of lesson must be watched)
        valid_lessons = []
        ...
        if not valid_lessons.empty:
            less_interacted_videos = valid_lessons[(valid_lessons["_pause"] + valid_lessons["_seek"]) < 3]
        ...
        # Scoring 2: Videos watched offline (pb_type == 2)
        ...
        # Scoring 3: Total sessions per lesson_id (session gap less than 1 minute)
        session_score = 10
        for lesson_id, lesson_group in group.groupby("lesson_id"):
...
if uploaded_file:
    ...
    if all(col in data.columns for col in required_columns):
        ...
        st.error(f"Missing columns: {', '.join(missing_columns)}. Ensure the dataset includes: {', '.join(required_columns)}")
